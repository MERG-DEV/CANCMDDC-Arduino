...
struct {
	byte    id;
	boolean enumerating;
	/* This could be made more memory-efficient by using one bit per CANID for the 110 possible ids.
...
void setup()
{
  ...
  canId.id = canId.id - deviceAddress;
...
void loop()
{
	...
	if (cbusActive == true)
	{
		...
		{
			if (nextMessage.canId == canId.id)                           // another node is using the same id, so force enum
			{
				flash_counter = 10;
				sendRTR();
			}
			...
			if (nextMessage.len == 0)                                    // check to see whether enum-frame is received.
			{
				if (canId.enumerating)
				{
					...
					canId.ids[canId.ptr] = nextMessage.canId;
					canId.ptr++;
				}
			}
...
void locoRequest(unsigned int address, byte long_address, byte flags)
{
  ...
  if (cancmd_present == FALSE)
  {
	...
	// Make the session id unique across all CANCMDDC instances, and non-zero.
	// As deviceAddress is 0-7, the MSB will always be zero.
    locoSession((deviceAddress << 4) + controllerIndex + 1, address, long_address, SF_FORWARDS, 0);
 #if DEBUG
...
void sendRTR()
{
	...
	if (cbusActive == true)
	{
		...
		canId.ptr = 0;
		CAN0.sendMsgBuf(CAN_RTR_MASK, 0, NULL);
		delay(1000);
	}
}
